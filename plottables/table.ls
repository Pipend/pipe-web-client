...
module.exports = ({Plottable, nv, plot-chart, plot}) -> new Plottable do 
    (view, result, {cell, cells, cols-order, cols}, continuation) !--> 
        ...
        cols = cols ? do -> 
            ...
            |> Obj.keys 
            |> (filter (.index-of \$) >> (!= 0)) 
            |> sort-by (-> b = (cols-order.index-of it); if b == -1 then Infinity else b )

        if d3.select view .select \pre .empty!
            d3.select view .append \pre .append \table .attr \class, \plottable
                ..append \thead .append \tr
                ..append \tbody

        $table = d3.select view .select \table
            ..select 'thead tr' .select-all \td .data cols
                ..enter!
                    .append \td
                ..exit!.remove!
                ..text id
                ..attr \class, id
            ..select \tbody .select-all \tr .data result
                ..enter!
                    .append \tr
                    .attr \style, (.$style)
                ..exit!.remove!
